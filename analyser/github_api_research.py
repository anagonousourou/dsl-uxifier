...
def save_main_file(repo_name,url,dir):
    ...
    response = urllib.request.urlopen(url.replace("https://github.com/","https://raw.githubusercontent.com/").replace("/blob",""))
...
def get_projects_with_topic_and_language(topic,language):
    ...
    r = requests.get(f"https://api.github.com/search/repositories?q=a+topic:{topic}+language:{language}&per_page=100",
...
def fetch_main_tf_for(topic,language,dir="",save=False,repos_name=None):
    ...
    for repos_name in tqdm(repos_name):
        ...
        if link: main_tf_links[repos_name] = link
